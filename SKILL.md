...
- 先讀 `memory.md`
- 再看 `app.py`、`index.html`、`Whisper 字幕神器.app`、`start.command`、`start.bat`
- 若是要確認目前狀態，macOS 一般使用優先雙擊 `Whisper 字幕神器.app`，除錯跑 `start.command`，Windows 優先跑 `start.bat`
...
- GPU / 裝置管理：`/device-info`、`/set-device`、`/cuda-diagnose`
...
- Windows/NVIDIA 再決定是否安裝 CUDA 版 PyTorch；macOS/Apple Silicon 優先看 MPS 狀態
