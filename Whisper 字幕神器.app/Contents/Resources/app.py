...
def ffmpeg_candidates() -> list[str]:
    ...
    if is_windows():
        ...
        app_data_bin = str(Path.home() / "Library" / "Application Support" / "WhisperSubtitle" / "bin")
        candidates.extend(
            [
                app_data_bin,
                "/opt/homebrew/bin",
...
setup_status_msg = "正在初始化..."
setup_is_done = False

@app.route("/api/setup-status")
def api_setup_status():
    global setup_status_msg, setup_is_done
    return jsonify({"msg": setup_status_msg, "done": setup_is_done})
...
def background_setup():
    ...
    try:
        ...
        def update_cb(msg):
            global setup_status_msg
            setup_status_msg = msg
            print(f"[SETUP] {msg}")

        setup_ml.download_ffmpeg_static(update_cb)
        setup_ml.install_ml_deps(update_cb)
        
        setup_status_msg = "準備完成，即將進入主程式！"
        setup_is_done = True
    except Exception as e:
        setup_status_msg = f"初始化發生錯誤: {e}"
        print(f"[ERROR] {setup_status_msg}")
...
if __name__ == "__main__":
    ...
    class Api:
        def save_file(self, default_filename, content_base64):
            import base64
            import subprocess
            from pathlib import Path
            ...
            try:
                ...
                file_path = downloads_dir / default_filename
                
                counter = 1
                while file_path.exists():
                    stem = Path(default_filename).stem
                    ext = Path(default_filename).suffix
                    file_path = downloads_dir / f"{stem}_{counter}{ext}"
                    counter += 1
                ...
                print(f"[ERROR] Save file failed: {e}")
                return False

    api = Api()

    # 立即創建視窗，避免 Dock icon 跳動太久
    window = webview.create_window(
        "Whisper 字幕神器", 
        html="<body style='font-family: -apple-system, sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; background: linear-gradient(135deg, #FFF0F0 0%, #FFF5E6 100%); margin: 0; color: #333;'><h2>正在啟動...</h2></body>", 
        width=1024, 
        height=768,
        js_api=api
    )
    ...
    def start_app():
        print("=" * 42)
        print("  Whisper 字幕神器啟動中 (pywebview)")
        print(f"  服務位置：{url}")
        print("=" * 42)
        
        server_thread = threading.Thread(target=app.run, kwargs={"host": "127.0.0.1", "port": port, "debug": False, "threaded": True}, daemon=True)
        server_thread.start()
        ...
        has_ffmpeg = shutil.which("ffmpeg") is not None or (is_macos() and any(ffmpeg_path_in_candidate(c).is_file() for c in ffmpeg_candidates()))
        
        if has_whisper and has_torch and has_ffmpeg:
            global setup_is_done
            setup_is_done = True
            window.load_url(f"{url}/")
        else:
            window.load_url(f"{url}/loading.html")
            threading.Thread(target=background_setup, daemon=True).start()

    webview.start(start_app)
    
    # Webview closed, exit python
    os._exit(0)
